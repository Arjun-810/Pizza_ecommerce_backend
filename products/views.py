...
class OAuthLoginInitiatorView(APIView):
    def get(self, request: Request, provider: str):
        provider_app = oauth.create_client(provider)
        redirect_url = settings.OAUTH_REDIRECT_URL.format(provider=provider)
        return provider_app.authorize_redirect(request, redirect_url)


class OAuthLoginAuthorizeView(APIView):
    def get(self, request: Request, provider: str):
        provider_app = oauth.create_client(provider)
        token = provider_app.authorize_access_token(request._request)
        user_info = token.get("userinfo")
        user_data = {
            "username": user_info.get("email"),
            "name": user_info.get("name"),
            "contact_number": user_info.get("mobile_number", None),
            "password": str(random.randbytes(5)),
        }
        user = User.objects.filter(username=user_data.get("username")).first()
        if user is not None:
            if user.is_active == 0:
                user_data["is_active"] = 1
                srlz_data = UserSerializer(instance=user, data=user_data, partial=True)
                if srlz_data.is_valid():
                    srlz_data.save()
                else:
                    return Response(
                        user_srlz.errors, status=status.HTTP_400_BAD_REQUEST
                    )
        else:
...
class Register(APIView):
    ...
    def post(self, request):
        ...
        if user_exist:
            ...
            return Response(
                {"msg": "User already exist"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CartView(APIView):
    def get(self, request):
        if "id" not in request.session:
            return Response(
                {"error": "User not Login"}, status=status.HTTP_401_UNAUTHORIZED
            )
        user = get_object_or_404(User, id=request.session["id"])
    ...
    def post(self, request):
        ...
        if srlz_data.is_valid():
            ...
            return Response(
                {"msg": "Successfully added items in cart"},
    ...
    def delete(self, request):
        ...
        try:
            ...
            return Response(
                {"msg": "Cart deleted successfully"}, status=status.HTTP_200_OK
            )
        except:
            return Response(
...
class CartOperation(APIView):
    def put(self, request, pk):
        ...
        if srlz_data.is_valid():
            srlz_data.save()
            return Response(
                {"msg": "Cart updated successfully"}, status=status.HTTP_200_OK
            )
        else:
            return Response(srlz_data.errors, status=status.HTTP_400_BAD_REQUEST)
...
class SimpleCheckout(APIView):
    ...
    def post(self, request):
        ...
        data = stripe.checkout.Session.create(
            ...
            success_url="https://pizzafront.thecv.in/success",
            cancel_url="https://pizzafront.thecv.in/cancel",
        )
        order_data = {}
...
class SaveOrder(APIView):
    def post(self, request):
        ...
        srlz_data = OrderPutSerialier(
            instance=data,
            data={"order_status": session["payment_status"]},
            partial=True,
        )
        if srlz_data.is_valid():
            srlz_data.save()
            if "id" in request.session:
                cart = ProductCart.objects.filter(
